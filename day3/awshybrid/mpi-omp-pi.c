...
{
        int             NoIntervals, interval, chunk, Noofthreads;
...
//              NoIntervals=1000;
        ...
        if (MyRank == Root)
                chunk = NoIntervals / Numprocs;


        MPI_Bcast(&chunk, 1, MPI_INT, Root, MPI_COMM_WORLD);

        /*printf("the chunk is %d",chunk);*/
